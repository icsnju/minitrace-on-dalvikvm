...
static u1* dexGetCoverageData(DvmDex* pDvmDex, const DexMethod* pDexMethod)
{
    if (pDexMethod->codeOff == 0)
        return NULL;
    if (pDvmDex->coverageData == NULL) {
        return NULL;
    }
    return (u1*) (pDvmDex->coverageData + pDexMethod->codeOff);
}
    ...
    Method* meth)
{
    ...
    if (pDexCode != NULL) {
        ...
        meth->insnsCoverageData = dexGetCoverageData(clazz->pDvmDex, pDexMethod);
    } else {
        /*
...
//xxx is this the right exception?  better than an assertion.
    ...
    if (gDvm.minitracingActive) {
        mtPostClassPrepare(clazz);
    }
...
static int mtDumpClass(void* vclazz, void* varg)
{
    const ClassObject* clazz = (const ClassObject*) vclazz;
    int i;
    ...
    if (!dvmIsInterfaceClass(clazz)) {
        for (i = 0; i < clazz->virtualMethodCount; i++) {
            mtPrintMethodCoverageData(varg, clazz, &clazz->virtualMethods[i]);
        }
        for (i = 0; i < clazz->directMethodCount; i++) {
            mtPrintMethodCoverageData(varg, clazz, &clazz->directMethods[i]);
        }
    }
    return 0;
}
...
void mtDumpAllClassesCoverage(void* args)
{
    dvmHashTableLock(gDvm.loadedClasses);
    dvmHashForeach(gDvm.loadedClasses, mtDumpClass, args);
    dvmHashTableUnlock(gDvm.loadedClasses);
}
