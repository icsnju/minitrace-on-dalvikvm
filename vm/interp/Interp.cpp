...
void updateInterpBreak(Thread* thread, ExecutionSubModes subMode, bool enable)
{
    ...
    do {
        ...
        if (newValue.ctl.subMode & kSubModeMiniTrace)
            newValue.ctl.breakFlags |= kInterpMiniTrace;
...
void mtDisableMiniTrace(Thread* thread)
{
    updateInterpBreak(thread, kSubModeMiniTrace, false);
}

void mtEnableMiniTrace(Thread* thread)
{
    updateInterpBreak(thread, kSubModeMiniTrace, true);
}
...
void dvmCheckBefore(const u2 *pc, u4 *fp, Thread* self)
{
    ...
    if (IS_METHOD_FLAG_SET(method, METHOD_ISMINITRACEABLE)) {
      if (method->insnsCoverageData != NULL) {
          method->insnsCoverageData[(pc - method->insns)] = 1;
      }
      if (self->interpBreak.ctl.breakFlags == kInterpMiniTrace) {
        return;
      }
    }
...
void dvmInterpret(Thread* self, const Method* method, JValue* pResult)
{
    ...
    // Local change to interpreter state
    if (gDvm.minitracingActive) {
        mtEnableMiniTrace(self);
    }
